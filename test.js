...
function buildChoices(repetition) {
    const choices = [];
    for (let i = 0; i < LENGTH; i += 1) {
        choices.push(repetition ? CHARACTERS : CHARACTERS - i);
    }
    return choices;
}
...
function render(repetition) {
    ...
    const choices = buildChoices(repetition);
...
function impactAnswerHTML() {
    const withRep = multiply(buildChoices(true));
    const withoutRep = multiply(buildChoices(false));
    const difference = withRep - withoutRep;
    const ratio = (withRep / withoutRep).toFixed(2);

    return `
        <p>Allowing repetition keeps every slot at 36 choices. Formula: 36^5 = ${withRep.toLocaleString()}.</p>
...
const answers = {
    drop: `Without repetition, each chosen character is removed from the pool. So the choices go 36, 35, 34, 33, 32. Formula: 36 x 35 x 34 x 33 x 32 = ${multiply(buildChoices(false)).toLocaleString()}.`,
    ...
    `,
    reason: `
        <p>For a 5-character password, fill positions one by one and multiply valid options per slot: Total = n1 x n2 x n3 x n4 x n5.</p>
